/*
 * 
 * 23) Escreva um algoritmo que leia o código de um aluno e suas três notas. 
 * Calcule a média ponderada do aluno, considerando que o peso para a maior nota seja 4 e 
 * para as duas restantes, 3. Mostre o código do aluno, suas três notas, 
 * a média calculada e uma mensagem "APROVADO" se a média for maior ou igual a 5 e
 * "REPROVADO" se a média for menor que 5. Repita a operação até que o código lido seja negativo.
 * 
*/

function notaDoAluno(codigoDoAluno, nota1, nota2, nota3) {
	let notas = [nota1, nota2, nota3];
	notas.sort((a, b) => a < b ? 1 : -1);
	
	let mediaFinal = 4 * notas[0] + 3 * notas[1] + 3 * notas[2];
	let resultado = (n) => n >= 5 ? "APROVADO" : "REPROVADO";

	return `Aluno ${codigoDoAluno}, com as notas ${nota1}, ${nota2}, ${nota3}, com a média de ${mediaFinal} está ${resultado(mediaFinal)}`;
}

console.log(notaDoAluno(123, 2.8, 6, 3.5));